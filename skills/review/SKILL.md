...
name: review
user-invocable: true
description: Runs quality review agents on demand — reviews code against project standards for architecture, tests, and simplicity, then writes one consolidated, numbered report.
...
2. Use provided paths as review scope. Derive the scope slug deterministically from the first
   ...
   `src/auth/token.ts` → `lib-auth-token`). Same paths always give the same slug.
...
- Because ids come from a deterministic sort (severity → file → line → rule), re-running on
  ...
  finding also carries a stable rule id (e.g. `project/missing-null-check`) the user can act on
  as a class.
