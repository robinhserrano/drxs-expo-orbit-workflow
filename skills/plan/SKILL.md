...
- **@best-practices-research-agent**: Researches and synthesizes best practices for the project's technology stack, following project conventions first, then official documentation, and finally industry standards.
...
- **Standard or Extensive template** → follow the [plan review procedure](references/plan-review.md) with `<PLAN_PATH>` set to the plan file. It runs the simplicity, the project, and scope-splitting agents in parallel, applies their findings to the plan inline, and — if the plan is too large for one PR — offers to restructure the work into phases or split it into part-N files.
