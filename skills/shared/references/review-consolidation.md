...
`project/missing-null-check`, `architecture/layer-violation`. The rule id is the finding's
...
|-------|----------|---------------|
| project-review-agent | the project | `project` |
...
- FINDING-01 · 🔴 Critical · `tests/missing-test-file` · AuthCubit has no test file (src/auth/authStore.ts)
- FINDING-02 · 🔴 Critical · `architecture/layer-violation` · Presentation imports data client directly (src/ui/Home.tsx:12)
- FINDING-03 · 🟡 Important · `project/force-unwrap` · Force-unwrap on a nullable token (src/auth/token.ts:20)
- …
