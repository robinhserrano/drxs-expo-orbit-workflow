...
|----|----------|------|----------|---------|
| FINDING-01 | 🔴 Critical | `tests/missing-test-file` | `src/auth/authStore.ts` | AuthCubit has no test file |
| FINDING-02 | 🔴 Critical | `architecture/layer-violation` | `src/ui/Home.tsx:12` | Presentation imports data client directly |
| FINDING-03 | 🟡 Important | `project/force-unwrap` | `src/auth/token.ts:20` | Force-unwrap on a nullable token |
| FINDING-04 | 🔵 Suggestion | `simplicity/inline-single-use` | `src/utils/format.ts:20` | Inline single-use helper |

## Critical

### FINDING-01 · `tests/missing-test-file` · `src/auth/authStore.ts`
...
### FINDING-02 · `architecture/layer-violation` · `src/ui/Home.tsx:12`
...
### FINDING-03 · `project/force-unwrap` · `src/auth/token.ts:20`
...
- **Reported by**: project-review-agent · [details](raw/project-review.md)
